...
class TestLuceneQueries(HttpTestCase):
    ...
    m={
       ...
       "query1":"kookaburra",
       "query2":"zebra",
       "sort_num":"min_length<number>",
       "sort_num_rev":"-min_length<number>",
       "sort_str":"latin_name<string>",
       "sort_str_rev":"-latin_name<string>",
       "group_by":"diet",
       "group_by_sort":"diet<string>",
       "group_by_sort_rev":"-diet<string>",
       "include_field":"min_length",
       "include_invalid_field":"wiki_page"
       }
    perfTesting=False
    ...
    def test_sort_ascending_number(self):
        ...
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    self.SEARCH_URL,
                                    params={'q': '*:*',
                                            'sort': json.dumps([self.m["sort_num"]])})
        assert_that(response.status_code, is_(helpers.accepted()))
    ...
    def test_sort_descending_number(self):
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    self.SEARCH_URL,
                                    params={'q': '*:*',
                                            'sort': json.dumps([self.m["sort_num_rev"]])})
        assert_that(response.status_code, is_(helpers.accepted()))
    ...
    def test_sort_ascending_string(self):
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    self.SEARCH_URL,
                                    params={'q': '*:*',
                                            'sort': json.dumps([self.m["sort_str"]])})
        assert_that(response.status_code, is_(helpers.accepted()))
    ...
    def test_sort_descending_string(self):
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    self.SEARCH_URL,
                                    params={'q': '*:*',
                                            'sort': json.dumps([self.m["sort_str_rev"]])})
        assert_that(response.status_code, is_(helpers.accepted()))
    ...
    def test_group_field_string_sort_ascending(self):
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    ...
                                    params={'q': '*:*',
                                            'group_field': self.m["group_by"],
                                            'group_sort': json.dumps([self.m["group_by_sort"]])})
        assert_that(response.status_code, is_(helpers.accepted()))
    ...
    def test_group_field_string_sort_descending(self):
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    ...
                                    params={'q': '*:*',
                                            'group_field': self.m["group_by"],
                                            'group_sort': json.dumps([self.m["group_by_sort_rev"]])})
        assert_that(response.status_code, is_(helpers.accepted()))
    ...
    def test_ranges(self):
        ...
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    self.SEARCH_URL.replace("animals", "length_range"),
                                    params={'q': '*:*',
    ...
    def test_counts(self):
        ...
        response = self.session.get(self._env['TESTY_DB_URL'] +
                                    self.SEARCH_URL.replace("animals", "class_count"),
    ...
    def test_query(self):
        ...
        if not self.perfTesting:
            assert_that(response.json()['total_rows'], is_(11))

    def test_include_fields(self):
        response = self.get_include_fields_wildcard_q(self.m["include_field"])

        assert_that(response.status_code, is_(helpers.accepted()))
        
        if not self.perfTesting:
            for row in response.json()['rows']:
                if row['id'] == "_design/"+self.viewName:
                    pass
                else:
                    assert_that(row['fields'].keys(), is_([unicode(self.m["include_field"])]))

    def test_include_fields_invalid_fields(self):
        response = self.get_include_fields_wildcard_q(self.m["include_invalid_field"])
